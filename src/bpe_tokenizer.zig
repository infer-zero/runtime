...
/// `Bpe.init`. The *semantic* interpretation of special tokens
...
pub const Vocabulary = struct {
    ...
    /// The vocabulary's designated end-of-sequence token. Snapshotted
    /// onto `Tokenizer.eos_token_id` by `Bpe.init`. Defaults to 0 —
    /// variants/adapters must populate it from the parsed tokenizer
    /// config at load time, BEFORE calling `Bpe.init`.
...
test "decodeToken returns token string" {
    ...
    const tokenizer = Bpe.init(vocab);
...
test "encode/decode via the Tokenizer interface" {
    const allocator = testing.allocator;
    var vocab = try createTestVocabulary(allocator);
    defer cleanupTestVocabulary(allocator, &vocab);

    var bpe = Bpe.init(vocab);
    const iface: *Tokenizer = &bpe.interface;

    const tokens = try iface.encode(allocator, "hello");
    defer allocator.free(tokens);
    try testing.expect(tokens.len > 0);

    // The leading MetaSpace doesn't merge in this toy vocab, so it
    // round-trips as <unk> — same as the "encode handles unknown
    // tokens" case above. The interface dispatch is what's under test.
    const text = try iface.decode(allocator, tokens);
    defer allocator.free(text);
    try testing.expectEqualStrings("<unk>hello", text);
}
